...
bool CClient::r_GetRef(LPCTSTR &pszKey, CScriptObj *&pRef)
{
	...
	int index = FindTableHeadSorted(pszKey, sm_szRefKeys, COUNTOF(sm_szRefKeys) - 1, sizeof(pszKey));
	if ( index >= 0 )
	{
...
bool CClient::r_WriteVal(LPCTSTR pszKey, CGString &sVal, CTextConsole *pSrc)
{
	...
	if ( !strnicmp("TARGP", pszKey, 5) && ((pszKey[5] == '\0') || (pszKey[5] == '.')) )
		...
		index = FindTableSorted(pszKey, sm_szLoadKeys, COUNTOF(sm_szLoadKeys) - 1, sizeof(pszKey));
...
bool CClient::r_LoadVal(CScript &s)
{
	...
	switch ( FindTableSorted(pszKey, sm_szLoadKeys, COUNTOF(sm_szLoadKeys) - 1, sizeof(pszKey)) )
	{
		case CC_ALLMOVE:
		{
...
bool CClient::r_Verb(CScript &s, CTextConsole *pSrc) // Execute command from script
{
	...
	int index = FindTableSorted(pszKey, sm_szVerbKeys, COUNTOF(sm_szVerbKeys) - 1, sizeof(pszKey));
	switch ( index )
	{
		case CV_ADD:
		{
