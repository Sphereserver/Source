...
void CClient::Event_ChatButton(const NCHAR *pszName)
{
	...
	if ( GetAccount()->m_sChatName.IsEmpty() )
	{
		...
		{
			// SA+ clients don't understand CHATMSG_GetChatName packet, so the name must be generated by the server.
...
void CClient::Event_Item_Drop(CGrayUID uidItem, CPointMap pt, CGrayUID uidOn, unsigned char gridIndex)
{
	...
	if ( pItem->IsType(IT_GAME_PIECE) )
	{
		...
		{
			CItemContainer *pGameBoard = dynamic_cast<CItemContainer *>(m_Targ_PrvUID.ItemFind());
			if ( pGameBoard )
...
void CClient::Event_TalkUNICODE(NWORD *wszText, int iTextLen, HUE_TYPE wHue, TALKMODE_TYPE mMode, FONT_TYPE font, LPCTSTR pszLang)
{
	...
	if ( !Event_Command(pszText, mMode) )
	{
		...
		if ( g_Log.IsLoggedMask(LOGM_PLAYER_SPEAK) )
			g_Log.Event(LOGM_PLAYER_SPEAK, "%lx:'%s' Says UNICODE '%s' '%s' mode=%d%s\n", GetSocketID(), m_pChar->GetName(), pAccount->m_lang.GetStr(), pszText, mMode, bCancelSpeech ? " (muted)" : "");
