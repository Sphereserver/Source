...
void CClient::Cmd_GM_Page( LPCTSTR pszReason ) // Help button (Calls GM Call Menus up)
{
	...
	sprintf(pszMsg, g_Cfg.GetDefaultMsg(DEFMSG_MSG_GMPAGE_REC), m_pChar->GetName(), static_cast<DWORD>(m_pChar->GetUID()), ptPlayerLocation.m_x, ptPlayerLocation.m_y, ptPlayerLocation.m_z, ptPlayerLocation.m_map, pszReason);
	...
	SysMessageDefault(fFound ? DEFMSG_MSG_GMPAGE_NOTIFIED : DEFMSG_MSG_GMPAGE_QUED);
	SysMessagef(g_Cfg.GetDefaultMsg(DEFMSG_MSG_GMPAGE_QNUM), static_cast<int>(g_World.m_GMPages.GetCount()));
...
void CClient::Cmd_GM_PageMenu( DWORD iEntryStart )
{
