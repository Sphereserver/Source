	...
	for ( int iTick = 0; iTick < TICK_PER_SEC; ++iTick )
	{
		...
		for ( it = m_timedFunctions[iTick].begin(); it != m_timedFunctions[iTick].end(); ++it )
		{
			...
			if ( tf->uid.IsValidUID() )
			{
				...
				s.WriteKeyFormat("TimerFNumbers", "%d,%" FMTDWORD ",%d", iTick, tf->uid.GetObjUID(), tf->elapsed);
			}
		}
	}
}
...
bool CWorld::SaveStage()
{
	...
	else if ( m_iSaveStage == static_cast<int>(m_SectorsQty) + 3 )
	{
		...
		snprintf(szTimeElapsed, sizeof(szTimeElapsed), "%lld.%04lld", static_cast<INT64>(TIME_PROFILE_GET_HI / 1000), static_cast<INT64>(TIME_PROFILE_GET_LO));
