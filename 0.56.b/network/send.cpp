...
PacketMessageASCII::PacketMessageASCII(const CClient* target, LPCTSTR pszText, const CObjBaseTemplate * source, HUE_TYPE hue, TALKMODE_TYPE mode, FONT_TYPE font) : PacketSend(XCMD_Speak, 42, PRI_NORMAL)
{
	...
	writeByte(static_cast<unsigned char>(mode));
...
PacketMovementAck::PacketMovementAck(const CClient* target) : PacketSend(XCMD_WalkAck, 3, g_Cfg.m_fUsePacketPriorities? PRI_HIGHEST : PRI_NORMAL)
{
	...
	writeByte(static_cast<unsigned char>(target->GetChar()->Noto_GetFlag(target->GetChar(), false, target->GetNetState()->isClientVersion(MINCLIVER_NOTOINVUL))));
	push(target);
}
...
PacketDragCancel::PacketDragCancel(const CClient* target, Reason code) : PacketSend(XCMD_DragCancel, 2, g_Cfg.m_fUsePacketPriorities? PRI_HIGH : PRI_NORMAL)
{
	...
	writeByte(static_cast<unsigned char>(code));
	push(target);
}
...
PacketDeathMenu::PacketDeathMenu(const CClient* target, Reason reason) : PacketSend(XCMD_DeathMenu, 2, PRI_NORMAL)
{
	...
	writeByte(static_cast<unsigned char>(reason));
	push(target);
}
...
PacketItemEquipped::PacketItemEquipped(const CClient* target, const CItem* item) : PacketSend(XCMD_ItemEquip, 15, PRI_NORMAL)
{
	...
	writeByte(0);
	writeByte(static_cast<unsigned char>(layer));
...
PacketSkills::PacketSkills(const CClient* target, const CChar* character, SKILL_TYPE skill) : PacketSend(XCMD_Skill, 15, g_Cfg.m_fUsePacketPriorities? PRI_LOW : PRI_NORMAL)
{
	...
	if (skill >= SKILL_MAX)
	{
		...
		for (size_t i = 0; i < g_Cfg.m_iMaxSkill; i++)
		{
			...
			writeByte(static_cast<unsigned char>(character->Skill_GetLock(static_cast<SKILL_TYPE>(i))));
			if (includeCaps)
	...
	else
	{
		...
		writeByte(static_cast<unsigned char>(character->Skill_GetLock(skill)));
		if (includeCaps)
...
PacketItemContents::PacketItemContents(const CClient* target, const CItem* spellbook) : PacketSend(XCMD_Content, 5, PRI_NORMAL), m_container(spellbook->GetUID())
{
	...
	for (int i = SPELL_Clumsy; i < SPELL_BOOK_QTY; i++)
	{
		...
		writeInt16(0);
		writeInt16(0);
		if (includeGrid)	writeByte(static_cast<unsigned char>(m_count));
		writeInt32(spellbook->GetUID());
		writeInt16(HUE_DEFAULT);
...
PacketQueryClient::PacketQueryClient(CClient* target, BYTE bCmd) : PacketSend(XCMD_StaticUpdate, 15, PRI_NORMAL)
{
	...
	switch (bCmd)
	{
		case 0x01:
		{
			...
			for (int i = 0; i < 2; i++)
			{
				writeByte(static_cast<unsigned char>(i));
...
PacketPlaySound::PacketPlaySound(const CClient* target, SOUND_TYPE sound, int flags, int volume, const CPointMap& pos) : PacketSend(XCMD_Sound, 12, PRI_NORMAL)
{
	...
	writeByte(static_cast<unsigned char>(flags));
...
PacketGameTime::PacketGameTime(const CClient* target, int hours, int minutes, int seconds) : PacketSend(XCMD_Time, 4, g_Cfg.m_fUsePacketPriorities? PRI_IDLE : PRI_NORMAL)
{
	...
	writeByte(static_cast<unsigned char>(hours));
	writeByte(static_cast<unsigned char>(minutes));
	writeByte(static_cast<unsigned char>(seconds));
	push(target);
}
...
void PacketEffect::writeBasicEffect(EFFECT_TYPE motion, ITEMID_TYPE id, const CObjBaseTemplate* dst, const CObjBaseTemplate* src, BYTE speed, BYTE loop, bool explode)
{
	...
	writeByte(static_cast<unsigned char>(motion));
...
PacketBulletinBoard::PacketBulletinBoard(const CClient* target, BBOARDF_TYPE action, const CItemContainer* board, const CItemMessage* message) : PacketSend(XCMD_BBoard, 20, g_Cfg.m_fUsePacketPriorities? PRI_LOW : PRI_NORMAL)
{
	...
	initLength();
	writeByte(static_cast<unsigned char>(action == BBOARDF_REQ_FULL ? BBOARDF_MSG_BODY : BBOARDF_MSG_HEAD));
	writeInt32(board->GetUID());
...
int PacketVendorBuyList::fillContainer(const CItemContainer* container, int convertFactor, size_t maxItems)
{
	...
	for (CItem* item = container->GetContentTail(); item != NULL && count < maxItems; item = item->GetPrev())
	{
		...
		writeByte(static_cast<unsigned char>(len));
	...
	size_t endpos = getPosition();
	seek(countpos);
	writeByte(static_cast<unsigned char>(count));
	seek(endpos);
...
PacketCharacterMove::PacketCharacterMove(const CClient* target, const CChar* character, BYTE direction) : PacketSend(XCMD_CharMove, 17, PRI_NORMAL)
{
	...
	writeByte(static_cast<unsigned char>(character->Noto_GetFlag(target->GetChar(), false, target->GetNetState()->isClientVersion(MINCLIVER_NOTOINVUL))));
 ...
 *
 ***************************************************************************/
PacketAddPrompt::PacketAddPrompt(const CClient* target, CGrayUID context1, CGrayUID context2, bool useUnicode) : PacketSend(static_cast<unsigned char>(useUnicode ? XCMD_PromptUNICODE : XCMD_Prompt), 16, g_Cfg.m_fUsePacketPriorities ? PRI_LOW : PRI_NORMAL)
{
...
void PacketServerList::writeServerEntry(const CServerRef& server, int index, bool reverseIp)
{
	...
	writeByte(static_cast<unsigned char>(percentFull));
...
PacketCharacterList::PacketCharacterList(CClient* target, const CChar* lastCharacter) : PacketSend(XCMD_CharList, 9, g_Cfg.m_fUsePacketPriorities? PRI_LOW : PRI_NORMAL)
{
	...
	writeByte(static_cast<unsigned char>(startCount));
	...
	if (includeExtraStartInfo)
	{
		...
		for (int i = 0; i < startCount; i++)
		{
			...
			writeByte(static_cast<unsigned char>(i + 1));
	...
	int flags = g_Cfg.GetPacketFlag(true, static_cast<RESDISPLAY_VERSION>(account->GetResDisp()), maximum(account->GetMaxChars(), static_cast<unsigned char>(account->m_Chars.GetCharCount())));
...
PacketGumpValueInput::PacketGumpValueInput(const CClient* target, bool cancel, INPVAL_STYLE style, DWORD maxLength, LPCTSTR text, LPCTSTR caption, CObjBase* object) : PacketSend(XCMD_GumpInpVal, 21, g_Cfg.m_fUsePacketPriorities? PRI_LOW : PRI_NORMAL)
{
	...
	writeBool(cancel);
	writeByte(static_cast<unsigned char>(style));
...
PacketSeason::PacketSeason(const CClient* target, SEASON_TYPE season, bool playMusic) : PacketSend(XCMD_Season, 3, PRI_NORMAL)
{
	...
	writeByte(static_cast<unsigned char>(season));
	writeBool(playMusic);
...
PacketPartyList::PacketPartyList(const CCharRefArray* members) : PacketParty(PARTYMSG_Add, 11, PRI_NORMAL)
{
	...
	writeByte(static_cast<unsigned char>(iQty));
...
PacketMapChange::PacketMapChange(const CClient* target, int map) : PacketExtended(EXTDATA_Map_Change, 6, PRI_NORMAL)
{
	...
	writeByte(static_cast<unsigned char>(map));
...
void PacketDisplayPopup::finalise(void)
{
	...
	seek(11);
	writeByte(static_cast<unsigned char>(m_popupCount));
...
PacketMessageLocalisedEx::PacketMessageLocalisedEx(const CClient* target, int cliloc, const CObjBaseTemplate* source, HUE_TYPE hue, TALKMODE_TYPE mode, FONT_TYPE font, AFFIX_TYPE affixType, LPCTSTR affix, LPCTSTR args) : PacketSend(XCMD_SpeakLocalizedEx, 52, PRI_NORMAL)
{
	...
	writeInt16(static_cast<WORD>(font));
	writeInt32(cliloc);
	writeByte(static_cast<unsigned char>(affixType));
...
bool PacketHouseDesign::writePlaneData(int plane, int itemCount, BYTE* data, int dataSize)
{
	...
	writeByte(static_cast<unsigned char>(plane | 0x20));
	writeByte(static_cast<unsigned char>(dataSize));
...
bool PacketHouseDesign::writeStairData(ITEMID_TYPE id, int x, int y, int z)
{
	...
	m_stairBuffer[m_stairCount].m_x = static_cast<unsigned char>(x);
	m_stairBuffer[m_stairCount].m_y = static_cast<unsigned char>(y);
	m_stairBuffer[m_stairCount].m_z = static_cast<unsigned char>(z);
	m_stairCount++;
...
void PacketHouseDesign::flushStairData(void)
{
	...
	writeByte(static_cast<unsigned char>(9 + m_stairPlaneCount));
	writeByte(static_cast<unsigned char>(stairSize));
	writeByte((BYTE)compressLength);
...
void PacketHouseDesign::finalise(void)
{
	...
	writeByte(static_cast<unsigned char>(m_planeCount + m_stairPlaneCount));
...
PacketItemWorldNew::PacketItemWorldNew(const CClient* target, CChar* mobile) : PacketItemWorld(XCMD_PutNew, 26, mobile->GetUID())
{
	...
	BYTE dir = static_cast<unsigned char>(mobile->m_dirFace);
