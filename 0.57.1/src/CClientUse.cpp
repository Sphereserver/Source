...
mustequipthis:
		...
		else if ( !pItem->IsType(IT_LIGHT_OUT) && !pItem->IsType(IT_LIGHT_LIT) && !pItem->IsTypeSpellbook())
		{
			goto mustequipthis;
		}
	}
	...
	case IT_SIGN_GUMP:
		// Things like grave stones and sign plaques. Need custom gumps.
		{
...
void CClient::Cmd_EditItem( CObjBase * pObj, int iSelect )
{
	...
	if ( iSelect > 0 )
	{
		...
		{
			OnTarg_Obj_Set( UID( m_tmMenu.m_Item[iSelect] ).ObjFind() );
		}
		return;
	}
...
bool CClient::Cmd_Skill_Tracking( int track_sel, bool fExec )
{
	...
	if ( track_sel ) // Not Cancelled
	{
		...
		int dist = m_pChar->Skill_GetBase(SKILL_TRACKING)/20 + 10;
		CWorldSearch AreaChars(m_pChar->GetTopPoint(), dist, m_pChar);
