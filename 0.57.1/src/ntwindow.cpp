...
public:
	...
	{
		m_pszAppName = SPHERE_TITLE "Server V" SPHERE_VERSION;
...
{
	TEMPSTRING(z);
	sprintf(z, "%s\n%s\nCompiled: %s", g_Serv.GetName(), SPHERE_TITLE " Version " SPHERE_VERSION, __DATE__ " " __TIME__);
					...
					if ( *zTemp && ( pos = strstr(zTemp, ".scp") ))
					{
						...
						*end = 0;
						...
						if ( *start )
						{
							...
							if ( strstr(SPHERE_FILE "tables.scp", start) )
							{
								TEMPSTRING(z);
								sprintf(z, "%s%s", g_Cfg.m_sSCPBaseDir, start);
...
LRESULT WINAPI CNTWindow::WindowProc( HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam )	// static
{
	try
	{
		...
		case WM_CLOSE:
			{
				...
				{
					if ( theApp.m_wndMain.MessageBox("Are you sure you want to close the server?", theApp.m_pszAppName, MB_YESNO|MB_ICONQUESTION) != IDYES )
						return 0;
				}
				PostQuitMessage(0);
		...
		case WM_DESTROY:
			...
			return 0;
		case WM_SETFOCUS:
			theApp.m_wndMain.m_wndInput.SetFocus();
			return 0;
		case WM_NOTIFY:
...
{
	// Unattach the window.
	if (( g_Serv.m_iExitFlag < 0 ) || ( g_Serv.m_iExitFlag >= 100 ))
	{
		TEMPSTRING(pszMsg);
		if ( g_Serv.m_iExitFlag < 100 )
			sprintf(pszMsg, "Server terminated by error %d!", g_Serv.m_iExitFlag);
		else
			sprintf(pszMsg, "Server exited by code %d!", g_Serv.m_iExitFlag);
		theApp.m_wndMain.MessageBox(pszMsg, theApp.m_pszAppName, MB_OK|MB_ICONEXCLAMATION);

		//	give 1 second to read this information and quit.
		NTWindow_OnTick( (g_Serv.m_iExitFlag < 0 ) ? 1000 : 10000 );
	}
}
...
bool NTWindow_PostMsg( LPCTSTR pszMsg )
{
	...
	if ( g_Serv.m_dwParentThread != CThread::Current() )
	{
