...
bool CItemVendable::IsValidSaleItem( bool fBuyFromVendor ) const
{
	...
	if ( ! IsMovableType())
	{
		g_Log.Warn("Vendor uid=0%lx selling unmovable item %s='%s'\n", GetTopLevelObj()->uid(), GetResourceName(), GetName());
		return false;
	}
	if ( ! fBuyFromVendor )
	{
...
bool CItemVendable::IsValidNPCSaleItem() const
{
	...
	{
		g_Log.Warn("Vendor uid=0%lx selling unpriced item %s='%s'\n", GetTopLevelObj()->uid(), GetResourceName(), GetName());
		return false;
	}
	return true;
}
