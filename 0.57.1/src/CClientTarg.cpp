...
bool CClient::OnTarg_Obj_Set( CObjBase * pObj )
{
	...
	// Parse the command.
	TEMPSTRING(pszLogMsg);
	if ( pObj->IsItem() )
	{
		CItem *pItem = static_cast <CItem*> (pObj);
		if ( pItem->GetAmount() > 1 ) sprintf(pszLogMsg, "'%s' commands uid=0%lx (%s) [amount=%d] to '%s'", GetName(), pObj->uid(), pObj->GetName(), pItem->GetAmount(), m_Targ_Text);
		else sprintf(pszLogMsg, "'%s' commands uid=0%lx (%s) to '%s'", GetName(), pObj->uid(), pObj->GetName(), m_Targ_Text);
	}
	else sprintf(pszLogMsg, "'%s' commands uid=0%lx (%s) to '%s'", GetName(), pObj->uid(), pObj->GetName(), m_Targ_Text);
...
int CClient::OnSkill_AnimalLore( UID uid, int iSkillLevel, bool fTest )
{
	// SKILL_ANIMALLORE
	...
	LPCTSTR pszText;
	int ifood = max(pChar->Stat_GetVal(STAT_FOOD), 7);
...
int CClient::OnSkill_ItemID( UID uid, int iSkillLevel, bool fTest )
{
	// SKILL_ITEMID
...
int CClient::OnSkill_EvalInt( UID uid, int iSkillLevel, bool fTest )
{
	// SKILL_EVALINT
...
int CClient::OnSkill_ArmsLore( UID uid, int iSkillLevel, bool fTest )
{
	// SKILL_ARMSLORE
...
int CClient::OnSkill_Anatomy( UID uid, int iSkillLevel, bool fTest )
{
	// SKILL_ANATOMY
	CChar * pChar = uid.CharFind();
	if ( pChar == NULL )
	...
	TEMPSTRING(pszTemp);
	sprintf(pszTemp, g_Cfg.GetDefaultMsg( DEFMSG_ANATOMY_RESULT ), pChar->GetName(),
...
int CClient::OnSkill_Forensics( UID uid, int iSkillLevel, bool fTest )
{
	// SKILL_FORENSICS
	...
	UID uidKiller( pCorpse->m_itCorpse.m_uidKiller );
...
int CClient::OnSkill_TasteID( UID uid, int iSkillLevel, bool fTest )
{
	// SKILL_TASTEID
...
int CClient::OnSkill_Info( SKILL_TYPE skill, UID uid, int iSkillLevel, bool fTest )
{
...
bool CClient::OnTarg_Skill_Magery( CObjBase * pObj, const CPointMap & pt )
{
	...
	m_pChar->m_Act_Targ					= pObj ? pObj->uid() : UID_CLEAR ;
	m_pChar->m_Act_p					= pt;
	m_Targ_p							= pt;
...
bool CClient::OnTarg_Pet_Command( CObjBase * pObj, const CPointMap & pt )
{
	...
	if ( m_tmPetCmd.m_fAllPets )
	{
		...
		CWorldSearch AreaChars(m_pChar->GetTopPoint(), UO_MAP_VIEW_SIGHT, m_pChar);
		while ( CChar *pCharPet = AreaChars.GetChar() )
		{
...
bool CClient::OnTarg_Pet_Stable( CChar * pCharPet )
{
	...
	TEMPSTRING(pszMsg);
	sprintf(pszMsg, g_Cfg.GetDefaultMsg(DEFMSG_STABLEMASTER_REM), pCharMaster->GetName());
...
CItem * CClient::OnTarg_Use_Multi( const CItemBase * pItemDef, const CPointMap & pt, WORD wAttr, HUE_TYPE wHue )
{
	...
	if ( pMultiDef != NULL && ! (wAttr&ATTR_MAGIC))
	{
		...
		CWorldSearch Area(pt, UO_MAP_VIEW_SIZE, m_pChar);
...
bool CClient::OnTarg_Party_Add( CChar * pChar )
{
	...
	*/
	...
	m_pChar->SetKeyNum("PARTY_LASTINVITE", pChar->uid());
