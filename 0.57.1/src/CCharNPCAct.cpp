...
bool CChar::NPC_OnVerb( CScript &s, CTextConsole * pSrc ) // Execute command from script
{
	...
	switch ( FindTableSorted( s.GetKey(), m_pNPC->sm_szVerbKeys, COUNTOF(m_pNPC->sm_szVerbKeys)-1 ))
	{
	case NV_BUY:
		...
		pCharSrc->GetClient()->addShopMenuBuy(this);
		break;
	case NV_BYE:
	...
	case NV_RUN:
		m_Act_p = GetTopPoint();
		m_Act_p.Move(GetDirStr(s.GetArgRaw(), this));
		NPC_WalkToPoint( true );
		break;
	case NV_SELL:
		...
		pCharSrc->GetClient()->addShopMenuSell(this);
		break;
	case NV_SHRINK:
		// we must own it.
...
bool CChar::NPC_Act_Begging( CChar * pChar )
{
	...
	else
	{
		...
		if ( !CanSee(pChar) )
		{
			if ( Calc_GetRandVal(75) > Stat_GetAdjusted(STAT_INT) )
				pChar = NULL;
		}
...
nopathfinding:
		...
		if ( ! CanMoveWalkTo(pMe, true, false, Dir ) )
		{
			...
			if ( ! CanMoveWalkTo(pMe, true, false, Dir ))
			{
				...
				else if (( g_Cfg.m_iNpcAi&NPC_AI_EXTRA ) && ( iInt > iRand ))
				{
					int			i;
					CPointMap	point;
					for ( i = 0; i < 2; i++ )
					{
						...
						{
							if ( abs(pItem->GetTopZ() - pMe.m_z) > 3 ) continue;		// item is too high
...
bool CChar::NPC_LookAtChar( CChar * pChar, int iDist )
{
	...
	if ( !m_pNPC || !pChar || ( pChar == this ) || !CanSeeLOS(pChar) )
		return false;
...
bool CChar::NPC_LookAround( bool fForceCheckItems )
{
	...
	if ( m_pNPC->m_Brain == NPCBRAIN_BERSERK || !Calc_GetRandVal(6) )	// Make some random noise
	{
	...
	CWorldSearch Area(GetTopPoint(), iRange, this);
...
bool CChar::NPC_FightMagery( CChar * pChar )
{
	...
	VariableList::Variable *pVar = GetKey("CASTSPELLS", true);
			...
			if ( ! pSpellDef->IsSpellType( SPELLFLAG_HARM ))
			{
				if ( pSpellDef->IsSpellType(SPELLFLAG_TARG_CHAR) && pSpellDef->IsSpellType(SPELLFLAG_GOOD) )
				{
					...
					if ( g_Cfg.m_iNpcAi&NPC_AI_EXTRA )
					{
						...
						while ( pTarget = AreaChars.GetChar() )
						{
...
bool CChar::NPC_OnItemGive( CChar * pCharSrc, CItem * pItem )
{
	...
	if ( NPC_IsVendor() )
	{
		...
		{
			CScriptTriggerArgs Args(pItem);
			this->r_Call("f_onsell_quick", pCharSrc, &Args);
		}
		return false;
	}
...
bool CChar::NPC_OnTickFood( int nFoodLevel )
{
	...
	if ( g_Cfg.m_iHitsHungerLoss && maxfood )
	{
		...
		if ( nFoodLevel <= maxfood/5 )
		{
			...
			else
			{
				...
				OnTakeDamage((max(Stat_GetMax(STAT_STR), 10) * loss)/100, NULL, DAMAGE_GOD);
			}
		}
	}
	...
	if ( IsStatFlag(STATF_Stone|STATF_Freeze|STATF_DEAD) )
		return false;
	SoundChar(CRESND_RAND2);
	return true;
}
...
void CChar::NPC_OnTickAction()
{
	...
	{
		int timeout	= max((150-Stat_GetAdjusted(STAT_DEX))/2, 0);
	...
	{
		EXC_SET("restocking");
		if ( CServTime::GetCurrentTime().GetTimeDiff(m_pNPC->m_timeRestock) > TICK_PER_SEC * 60 * 10 )
...
void CChar::NPC_Pathfinding()
{
	...
	CPathFinder	path(this, pTarg, iInt / 6);
	...
	for ( int i = 0; true; i++ )
	{
		if (( path.m_xPath == pTarg.m_x ) && ( path.m_yPath == pTarg.m_y )) break;
		if ( i >= MAX_NPC_PATH_STORAGE_SIZE ) break;

		path.ReadStep(i + 1);
		m_pNPC->m_nextX[i] = path.m_xPath;
		m_pNPC->m_nextY[i] = path.m_yPath;
	}

	EXC_CATCH;
	EXC_DEBUG_START;
	g_Log.Debug("'%s' point '%d,%d,%d,%d' [0%lx]\n", GetName(), local.m_x, local.m_y, local.m_z, local.m_map, GetUID());
	EXC_DEBUG_END;
}
...
void CChar::NPC_Food()
{
	...
	CWorldSearch AreaItems(GetTopPoint(), min(iSearchDistance,m_pNPC->m_Home_Dist_Wander));
	while ( CItem *pItem = AreaItems.GetItem() )
	{
					...
					case NPCACT_FLEE:
						{
							...
							CPointMap pt = g_World.FindItemTypeNearby(GetTopPoint(), IT_GRASS, min(iSearchDistance,m_pNPC->m_Home_Dist_Wander), true);
...
void CChar::NPC_AI()
{
	...
	int intToActMore = config.get("ai.tick.intnextaction");
	if ( iDex < Calc_GetRandVal(intToActMore) )
		return;

	NPCBRAIN_TYPE	simpleBrain = GetNPCBrain(true);
	CItemContainer *backpack = GetPack();		// can be NULL!
	bool inDark = pSector->IsDark();

	while ( iter < config.get("ai.tick.maxactions") )
	{
		bActed = false;
		iter++;
		...
		switch ( m_pNPC->m_Brain )
		{
			...
			case NPCBRAIN_STABLE:				//	Stable Man
			{
				...
				while ( CChar *pChar = Area.GetChar() )
				{
					...
					if ( pChar->IsClient() )
					{
						if (( iDist < 3 ) && !Calc_GetRandVal(300) )
						{
					...
					if ( iMaxFood && ( iFood < iMaxFood/4 ))
					{
						if ( iDist > 1 )
						{
							UpdateDir(pChar);
							UpdateAnimate(6);
							pChar->UpdateAnimate(3);
							pChar->Stat_SetVal(STAT_FOOD, iFood + Calc_GetRandVal2(1, iMaxFood - iFood));
							Emotef(g_Cfg.GetDefaultMsg(DEFMSG_STABLEMASTER_FEED), pChar->GetName());
						}
						else
						{
		...
		if ( !bActed && ( simpleBrain == NPCBRAIN_HUMAN ) && backpack )
		{
			//	Operate with items i do have in my pack

			//	equip/unequip light source if not equipped
			EXC_SET("light source");
			if ( bActed || IsStatFlag(STATF_War|STATF_NightSight) ) ;
			else if ( inDark )
			{
				CItem *pLightSource = LayerFind(LAYER_HAND2);
				if ( !pLightSource )
				{
					pLightSource = backpack->ContentFind(RESOURCE_ID(RES_TYPEDEF,IT_LIGHT_OUT));
					if ( pLightSource )
					{
						if ( ItemEquip(pLightSource) )
							bActed = true;
					}
				}
				else if ( pLightSource->IsType(IT_LIGHT_OUT) )
				{
					Use_Obj(pLightSource, false, true);
					bActed = true;
				}
			}
			else
			...
			else if ( IsStatFlag(STATF_War) )
			{
				...
				if ( !pWeapon || !pWeapon->IsTypeWeapon() )
				{
					...
					if ( !pWeapon || !pWeapon->IsTypeWeapon() )
					{
						CItem *pItem = backpack->ContentFind(RESOURCE_ID(RES_TYPEDEF, IT_SHIELD));
						if ( pItem )
							ItemEquip(pItem);
		...
		if (( iInt < intToActMore ) || ( Calc_GetRandVal(iInt) < intToActMore ))
			break;
	}
