...
enum OF_TYPE
{
	OF_Archery_CanMove	= 0x0001,
	OF_Items_AutoName	= 0x0002,
	OF_FileCommands		= 0x0004,
	OF_NoItemNaming		= 0x0008,
	OF_HouseMuteSpeech	= 0x0010,
};

enum EF_TYPE
{
	EF_Intrinsic_Locals			= 0x0002,
	EF_Item_Strict_Comparison	= 0x0004,
	EF_WalkCheck				= 0x0008,
	EF_Script_Profiler			= 0x0010
};

#define VALID_OF	(OF_Archery_CanMove|OF_Items_AutoName|OF_FileCommands|OF_NoItemNaming|OF_HouseMuteSpeech)
#define VALID_EF	(EF_Intrinsic_Locals|EF_Item_Strict_Comparison|EF_WalkCheck|EF_Script_Profiler)

enum MAGICFLAGS_TYPE
{
	MAGICF_NODIRCHANGE		= 0x0001,	// not rotate player when casting/targetting
	MAGICF_PRECAST			= 0x0002,	// use precasting
	MAGICF_IGNOREAR			= 0x0004,	// magic ignore ar
	MAGICF_CANHARMSELF		= 0x0008,	// i can do damage on self
	MAGICF_STACKSTATS		= 0x0010,	// allow multiple stat spells at once
	MAGICF_FREEZEONCAST		= 0x0020,	// freeze on cast // TODO: implement it
};
...
#define	DAMAGE_COLD			0x1000	// cold damage
#define DAMAGE_ENERGY		0x2000	// energy damage
...
private:
	...
	CGTypedArray<UID, UID>	m_uidCharArray;
public:
...
private:
	int ServPageRequest(CClient *pClient, LPCTSTR pszURLArgs);
public:
	...
	static bool ServPage(CClient *pClient, TCHAR *pszPage);
...
public:
	...
	int	 m_fUseHTTP;
	CGString m_sWorldBaseDir;	// "worldsave\" = world files go here.
...
#define COMBAT_ALLOWHITFROMSHIP		0x0004	// allow attacking opponents from ships
...
private:
	RESOURCE_ID ResourceGetNewID( RES_TYPE restype, LPCTSTR pszName, VariableList::Variable ** ppVarNum, bool fNewStyleDef );
...
public:
	...
	CServerDef *Server_GetDef( int index );
...
public:	
	...
	CObjBase *	m_pObj;
	vector<Element>	m_controls;
	vector<Element>	m_text;
	int			m_iTexts;
	int			m_iControls;
	int			m_x;
	int			m_y;
