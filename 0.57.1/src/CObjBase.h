...
enum MEMORY_TYPE
{
	...
	MEMORY_IAGGRESSOR	= 0x0008,	// I was the aggressor here. (good or evil)
	...
	MEMORY_AGGREIVED	= 0x0080,	// I was attacked and was the innocent party here !
...
enum NPC_MEM_ACT_TYPE	// A single primary memory about the object.
{
...
private:
	...
	PacketPropertyHash	*m_propertyHash;
	PacketPropertyList	*m_propertyList;

protected:
	CResourceRef m_BaseRef;	// Pointer to the resource that describes this type.
public:
	static const char *m_sClassName;
	VariableList m_TagDefs;		// attach extra tags here.

	CResourceRefArray m_OEvents;
	VariableList *GetTagDefs()
	{
		return &m_TagDefs;
	}
...
public:
	PacketPropertyHash	&getPropertyHashPacket();
	PacketPropertyList	&getPropertyPacket();
	virtual void initPropertyPackets();				// callback to trigger the reset of property packet
...
public:
	...
	void _cdecl Emotef(LPCTSTR format, ...);
	...
	void UpdateCanSee(PacketSend *packet, CClient *exclude = NULL) const;
...
enum ITRIG_TYPE
{
	...
	ITRIG_Click=1,
	ITRIG_CLIENTTOOLTIP, // Sending tool tip to client for this item
...
#define ATTR_STATIC			0x8000	// WorldForge merge marker. (used for statics saves)
	...
	UID m_uidLink;		// Linked to this other object in the world. (owned, key, etc)
	...
	union // 4(more1) + 4(more2) + 5(morep) = 13 bytes
	{
		...
		{
			UIDBase m_lockUID;		// more1=the lock code. normally this is the same as the uid (magic lock=non UID)
		...
		struct
		{
			UIDBase m_lockUID;		// more1 = the lock code. Normally this is the UID, except if uidLink is set.
		} m_itKey;
		...
		struct
		{
			WORD m_Hits_Cur;		// more1l= equiv to quality of the item (armor/weapon).
			WORD m_Hits_Max;		// more1h= can only be repaired up to this level.
		...
		struct
		{
			CServTime	m_timeDeath;	// more1=Death time of corpse object. 0=carved
			UIDBase m_uidKiller;	// more2=Who killed this corpse, carved or looted it last. sleep=self.
		...
		struct
		{
			...
			UIDBase m_UID;		// more2 = If stored by the stables. (offline creature)
		} m_itFigurine;
		...
		struct
		{
			UIDBase m_UIDCreator;	// more1 = who created this house or ship ?
...
public:
	...
	void  Flip();
	bool  LoadSetContainer( UID uid, LAYER_TYPE layer );
...
public:
	...
	int ContentCount( RESOURCE_ID_BASE rid, DWORD dwArg = 0 , bool bTrade = false );
	int ContentCountAll() const;
	int ContentConsume( RESOURCE_ID_BASE rid, int iQty = 1, bool fTest = false, DWORD dwArg = 0 , bool bTrade = false );

	int ResourceConsume( const CResourceQtyArray * pResources, int iReplicationQty, bool fTest = false, DWORD dwArg = 0 , bool bTrade = false );
...
public:
	...
	int Guild_SetLoyalTo( UID uid );
	UID Guild_GetLoyalTo() const;
...
private:
	...
	UID m_uidLinkTo;			// My char uid or enemy stone UID

	// Only apply to members.
	UID m_uidLoyalTo;	// Who am i loyal to? invalid value = myself.
...
public:
	static const char *m_sClassName;
	CStoneMember( CItemStone * pStone, UID uid, STONEPRIV_TYPE iType, LPCTSTR pTitle = "", UID loyaluidLink = 0, bool fArg1 = false, bool fArg2 = false, int nAccountGold = 0);
	...
	UID GetLinkUID() const { return m_uidLinkTo; }
...
private:
	...
	LPCTSTR GetCharter(int iLine) const { return m_sCharter[iLine]; }
	void SetCharter( int iLine, LPCTSTR pCharter ) { m_sCharter[iLine] = pCharter; }
...
private:
	...
	void WeDeclarePeace(UID uidEnemy, bool fForcePeace = false);
...
//	default to 18 steps, will have 18*4 extra bytes per char
#define MAX_NPC_PATH_STORAGE_SIZE	UO_MAP_VIEW_SIZE
...
private:
	...
	CResourceRef m_SkillClass;	// RES_SKILLCLASS SkillClass What skill class group have we selected.
	CResourceRef m_Profession;	// RES_PROFESSION Profession What profession have we selected
...
public:
	...
	long	m_pts;				// rating points
	unsigned int		m_curFollower;		// current followers
	unsigned int		m_maxFollower;		// maximum followers
	unsigned int		m_speedMode;		// speed mode (0x0 = Normal movement, 0x1 = Fast movement, 0x2 = Slow movement, 0x3 and above = Hybrid movement)
	int		m_resFire;			// fire resistance
	int		m_resCold;			// cold resistance
	int		m_resPoison;		// poison resistance
	int		m_resEnergy;		// energy resistance
	int		m_luck;				// luck
	int		m_iTithingPoints;	// paladin stuff

	static LPCTSTR const sm_szLoadKeys[];
	vector<Quest *>	m_quests;			// list of current quests
	vector<Quest *> m_questsHistory;	// list of all completed quests
...
public:
	...
	SkillClass *GetSkillClass() const;

	bool SetProfession(CChar *pChar, RESOURCE_ID rid);
...
public:
	...
	vector<TimedFunction>	m_timedFunctions;
	...
	UID m_uidWeapon;		// current Wielded weapon.	(could just get rid of this ?)
	...
	UID	m_Act_Targ;			// Current combat/action target
	UID	m_Act_TargPrv;		// Targeted bottle for alchemy or previous beg target.
	...
	union	// arg specific to the action type.(m_Act_SkillCurrent)
	{
		...
		struct
		{
			UIDBase m_FigurineUID;	// This creature is being ridden by this object link. IT_FIGURINE IT_EQ_HORSE
		} m_atRidden;
...
public:
	...
	VariableList::Variable *GetKey(LPCTSTR pszKey, bool fDef) const
	{
		VariableList::Variable *pVar = m_TagDefs.GetKey(pszKey);
...
public:
	...
	int translateAnimation(int action);
	void UpdateAnimate(int action, bool fBackward = false, BYTE iFrameDelay = 1, int frameCount = DIR_ANIM_QTY);
...
protected:
	void OnRemoveOb( CGObListRec* pObRec );	// Override this = called when removed from list.
	void OnToggleEquip(CItem *pItem, bool bToEquip);	// catches equip/unequip
public:
...
public:
	...
	bool Skill_Tracking( UID uidTarg, DIR_TYPE & dirPrv, int iDistMax = SHRT_MAX );
	bool Skill_MakeItem( ITEMID_TYPE id, UID uidTarg, SKTRIG_TYPE stage, bool fSkillOnly = false );
...
private:
	...
	CItemMemory * Memory_CreateObj( UID uid, WORD MemTypes );
...
public:
	...
	CItemMemory * Memory_FindObj( UID uid ) const;
	...
	CItemMemory * Memory_AddObjTypes( UID uid, WORD MemTypes );
	...
	CItemMemory * Memory_AddObj( UID uid, WORD MemTypes )
	{
...
public:
	CChar *Use_Figurine(CItem *pItem);
	CItem * Make_Figurine( UID uidOwner, ITEMID_TYPE id = ITEMID_NOTHING );
	CItem * NPC_Shrink();
...
public:
	...
	void Lock()
	{
		CThreadSafe *safe = static_cast <CThreadSafe*>(static_cast <CObjBase*>(this));
		safe->Lock();
	}
	void Release()
	{
