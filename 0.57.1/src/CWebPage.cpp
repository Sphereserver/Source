	...
	switch ( iHeadKey )
	{
		case WV_WEBPAGE:
			...
			// serv a web page to the pSrc
				...
				if ( pClient == NULL )
					return false;
				return ServPage(pClient, s.GetArgStr());
			}
			return true;
...
bool CWebPageDef::WebPageUpdate( bool fNow, LPCTSTR pszDstName, CTextConsole * pSrc )
{
	...
	{
		g_Log.Error("Can't open web page output '%s'\n", pszDstName);
		return false;
	}
	...
	while ( FileRead.ReadTextLine( false ))
	{
		...
		if ( fScriptMode )
		{
			...
			if ( pszFormat[0] != '\0' )
			{
				...
				{
					g_Log.Error("Web page source format error '%s'\n", pszTmp);
					continue;
				}
			}
...
bool CWebPageDef::SetSourceFile( LPCTSTR pszName, CClient * pClient )
{
	...
	if ( pClient == NULL )
	{
		...
		{
			g_Log.Error("Can't open web page input '%s'\n", m_sSrcFilePath);
			return false;
		}
...
int CWebPageDef::ServPageRequest(CClient *pClient, LPCTSTR pszURLArgs)
{
	...
	const char *sDate = datetime.FormatGmt();	// current date.
...
bool CWebPageDef::ServPage(CClient *pClient, TCHAR *pszPage)
{
	...
	if ( pWebPage )
	{
		iError = pWebPage->ServPageRequest(pClient, szPageName);
		if ( ! iError )
			return true;
	}
	...
	if ( iError == 404 )
	{
		...
		{
			if ( !tmppage.ServPageRequest(pClient, szPageName) )
				return true;
		}
	}
	...
	if ( pWebPage )
	{
		if ( !pWebPage->ServPageRequest(pClient, pszPage) )
			return true;
	}
	...
	CGTime datetime = CGTime::GetCurrentTime();
	const char *sDate = datetime.FormatGmt();
	CGString sMsgHead;
	CGString sText;
	...
	pClient->xSend((LPCTSTR)sMsgHead, sMsgHead.GetLength());
	return false;
}
