...
LPCTSTR CGTime::Format() const
{
	TEMPSTRING(pszTemp);
	return formattime(pszTemp, localtime(&m_time), TIME_LONG);
}

LPCTSTR CGTime::FormatGmt() const
{
	TEMPSTRING(pszTemp);
	return formattime(pszTemp, gmtime(&m_time), TIME_LONG|TIME_GMT);
}
