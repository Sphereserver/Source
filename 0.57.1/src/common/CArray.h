...
class CGObList : public CThreadSafe	// generic list of objects based on CGObListRec.
{
...
class CGTypedArray : public CThreadSafe
{
...
{
	Lock();
	if ( bElements && GetRealCount() )
		DestructElements( GetBasePtr(), GetRealCount() );
	Empty();
	Release();
}
...
int CGObSortArray<TYPE, KEY_TYPE>::AddSortKey( TYPE pNew, KEY_TYPE key )
{
	...
	if ( !iCompareRes )
	{
		...
		index = AddPresorted(index, iCompareRes, pNew);
	Release();
	return index;
}
