...
CResourceScript * CResourceBase::AddResourceFile( LPCTSTR pszName )
{
	...
	if ( ! strnicmp( pszTitle, SPHERE_FILE "tables", strlen(SPHERE_FILE "tables")))
	{
		// Don't dupe this.
		return NULL;
	}
...
bool CResourceDef::SetResourceName( LPCTSTR pszName )
{
	...
	for ( int i=0; pszName[i]; i++ )
	{
		if ( i >= maxLen )
		{
			g_Log.Error("Too long DEFNAME=%s\n", pszName);
			return false;
		}
		if ( !_ISCSYM(pszName[i]) )
		{
			g_Log.Error("Bad chars in DEFNAME=%s\n", pszName);
			return false;
		}
	}
	...
	VariableList::Variable *pVarKey = g_Exp.m_VarDefs.GetKey(pszName);
	if ( pVarKey )
	{
	...
	SetResourceVar(g_Exp.m_VarDefs.GetAt(iVarNum));
	return true;
}
...
bool CResourceScript::CheckForChange()
{
	...
	{
		g_Log.Error("Can't get stats info for file '%s'.\n", GetFilePath());
		return false;
	}
...
CResourceLink::CResourceLink(RESOURCE_ID rid, const VariableList::Variable *pDef) :
		...
		if ( m_pScript->IsKeyHead( "ON", 2 ))
		{
			...
			if ( iQty )
			{
				...
				else
				{
					...
					{
						g_Log.Error("Duplicate trigger '%s' in '%s'.\n", ppTable[iTrigger], GetResourceName());
						continue;
					}
				}
			}
...
bool CResourceLink::ResourceLock( CResourceLock &s )
{
	...
	// ret = -2 or -3
	g_Log.Error("ResourceLock '%s':%d id=%s FAILED\n", s.GetFilePath(), m_Context.m_lOffset, GetResourceName());
	return false;
}
...
void CScriptFileContext::OpenScript( const CScript * pScriptContext )
{
	...
	if ( thread )
		m_pPrvScriptContext = thread->ScriptContext(pScriptContext);
}
...
void CScriptFileContext::Close()
{
	if ( m_fOpenScript )
	{
		...
		CThread *thread = CThread::Thread();
		if ( thread )
			thread->ScriptContext(m_pPrvScriptContext);
	}
}
...
bool CResourceQty::Load(LPCTSTR &pszCmds)
{
	...
	if ( *pszCmds == '\0' )
	{
		g_Log.Error("Bad resource list element in '%s'\n", orig);
		return false;
	}
	...
	{
		g_Log.Error("Bad resource list id '%s'\n", orig);
		return false;
	}
