...
enum RES_TYPE	// all the script resource blocks we know how to deal with !
{
	...
	RES_PROFESSION,		// Character profession
...
struct RESOURCE_ID_BASE : public UIDBase
{
...
public:
	...
	CResourceQtyArray(LPCTSTR pszCmds = NULL);
...
protected:
	const VariableList::Variable *m_pDefName;	// The name of the resource. (optional)
public:
	static const char *m_sClassName;
	...
	CResourceDef(RESOURCE_ID rid, const VariableList::Variable *pDefName = NULL) :
		m_rid( rid ),
		m_pDefName( pDefName )
	{
	}
	...
	void SetResourceVar(const VariableList::Variable *pVarNum)
	{
...
public:
	...
	CResourceLink(RESOURCE_ID rid, const VariableList::Variable* pDef = NULL);
};
