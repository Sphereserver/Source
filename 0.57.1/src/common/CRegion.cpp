...
bool CTeleport::RealizeTeleport()
{
	...
	{
		g_Log.Error("CTeleport bad coords %s\n", WriteUsed());
		return false;
	}
...
bool CRegionBase::RealizeRegion()
{
	...
	for ( long l = 0; l < g_MapList.GetSectorQty(m_pt.m_map); l++ )
	{
		...
		if ( pSector && IsOverlapped(pSector->GetRect()) )
		{
			...
			{
				g_Log.Error("Linking sector #%d for map %d for region %s failed (fatal for this region).\n", l, m_pt.m_map, GetName());
				return false;
			}
			m_iLinkedSectors++;
		}
...
{
	EXC_TRY("WriteVal");
	RC_TYPES index = (RC_TYPES) FindTableHeadSorted( pszKey, sm_szLoadKeys, COUNTOF( sm_szLoadKeys )-1 );
	if ( index < 0 )
	{
...
{
	EXC_TRY("LoadVal");
	RC_TYPES index = (RC_TYPES) FindTableSorted( s.GetKey(), sm_szLoadKeys, COUNTOF( sm_szLoadKeys )-1 );
	if ( index < 0 )
	{
...
void CRegionWorld::r_WriteModified( CScript &s )
{
	...
	if ( m_iModified & REGMOD_TAGS )
	{
		...
		{
			sprintf(z, "TAG.%s", m_TagDefs.GetAt(j)->GetKey());
			if ( !strcmpi(z, "TAG.GUARDOWNER") ) continue;
			s.WriteKey(z, m_TagDefs.GetAt(j)->GetValStr());
		}
	}
}
