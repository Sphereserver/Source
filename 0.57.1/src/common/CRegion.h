...
private:
	UID	m_UID;		// How the server will refer to this. 0 = static item
...
public:
	...
	DWORD uid() const				{	return m_UID.GetObjUID(); }
...
public:
	...
	void UnionRect( const CGRect & rect )
	{
		...
		if ( m_map != rect.m_map )
			g_Log.Error("Uniting regions from different maps!\n");
	}
...
enum REGION_FLAGS
{
#define ADD(_a_,_b_,_c_)	_b_ = _a_,
