...
CResource::CResource()
{
	...
	m_iColorNotoCriminal = 0x03b2;		// gray (criminal)
	m_iColorNotoDefault = 0x03b2;		// gray (if not any other)
	...
	m_iColorNotoNeutral = 0x03b2;		// gray (can be attacked)
}
...
CResource::~CResource()
{
	...
	for ( int k = 0; k < PLEVEL_QTY; k++ )
		m_PrivCommands[k].RemoveAll();
	m_MoonGates.Empty();
...
{
	char buf[128];
	const char *sep = strchr(pszKey, '(');
	if ( sep == NULL )
		sep = strchr(pszKey, '.');
	if ( sep == NULL )
		return false;

	strcpylen(buf, pszKey, sep-pszKey+1);

	int iResType = FindTableSorted(buf, sm_szResourceBlocks, RES_QTY);
	if ( iResType < 0 ) return false;

	// Now get the index.
	pszKey = sep+1;
	if ( *pszKey == '\0' ) return false;
	...
	{
		LPCTSTR p = buf;
		pRef = CCharBase::FindCharBase((CREID_TYPE)Exp_GetVal(p));
	}
	else if ( iResType == RES_ITEMDEF )
	{
		LPCTSTR p = buf;
		pRef = CItemBase::FindItemBase((ITEMID_TYPE)g_Cfg.ResourceGetIndexType(RES_ITEMDEF, p));
	}
	else if ( iResType == RES_SPELL && ( *buf == '-' ))
	{
		pszKey++;
		LPCTSTR p = buf+1;
		int order = Exp_GetVal(p);
		pRef = m_SpellDefs_Sorted.IsValidIndex(order) ? m_SpellDefs_Sorted[order] : NULL;
	}
	else
	{
		LPCTSTR p = buf;
		RESOURCE_ID rid = ResourceGetID((RES_TYPE)iResType, p);
...
bool CResource::r_LoadVal( CScript &s )
{
	...
	if ( i < 0 )
	{
		...
		else if ( s.IsKeyHead("MAP", 3) )		//	MAPx=settings
		{
			...
			if (( *text < '0' ) || ( *text > '9' ))
				return false;
			return g_MapList.Load(ATOI(text), s.GetArgRaw());
		}
		else if ( s.IsKeyHead("PACKET", 6) )	//	PACKETx=<function name to execute upon packet>
		{
			...
			if (( index >= 0 ) && ( index <= 255 ))
			{
				...
				if ( !args || ( strlen(args) > 30 ))
					g_Log.Error("Invalid function name for packet filtering (limit is 30 chars).\n");
				else
				{
				...
				g_Log.Error("Packet filtering index %d out of range [0..254]\n", index);
		}
	...
	switch (i)
	{
		...
		case RC_HITSUPDATERATE:
			...
			break;
		case RC_LOG:
			g_Log.LogDir(s.GetArgStr());
			break;
		case RC_LOGMASK:
			g_Log.LogMask(s.GetArgVal());
			break;
		case RC_MULFILES:
...
bool CResource::r_WriteVal( LPCTSTR pszKey, CGString & sVal, CTextConsole * pSrc )
{
	...
	switch (i)
	{
		...
		case RC_GUARDLINGER:
			...
			break;
		case RC_HEARALL:
			sVal.FormatVal(g_Log.LogMask() & LOGM_PLAYER_SPEAK);
			break;
		case RC_HITSUPDATERATE:
		...
		case RC_RTIME:
			{
				if ( pszKey[5] != '.' )
					sVal = CGTime::GetCurrentTime().Format();
				else
				{
					pszKey += 5;
					SKIP_SEPARATORS(pszKey);
					if (!strnicmp("FORMAT",pszKey,6))
					{
						...
						GETNONWHITESPACE(pszKey);
						TEMPSTRING(z);
						strftime(z, 1024, pszKey, CGTime::GetCurrentTime().GetLocalTm());
						sVal = z;
					}
				}
			} break;
...
{
	for ( int i = 0; i < COUNTOF(g_Stat_Name); i++ )
	{
		if ( !strcmpi(g_Stat_Name[i], pszKey) )
			return (STAT_TYPE)i;
	}
	return (STAT_TYPE)-1;
}
...
CServerDef *CResource::Server_GetDef( int index )
{
	return m_Servers.IsValidIndex(index) ? static_cast <CServerDef*>(m_Servers[index]) : NULL;
}
...
bool CResource::LoadResourceSection( CScript * pScript )
{
	...
	VariableList::Variable *pVarNum = NULL;
	RESOURCE_ID rid;
	...
	if (( restype == RES_WORLDSCRIPT ) || ( restype == RES_WS ))
	{
		...
		VariableList::Variable *pVarBase	= g_Exp.m_VarDefs.GetKey( pszDef );
		VariableList::Variable *pVarNum	= NULL;
		if ( pVarBase )
			pVarNum				= pVarBase;
		if ( !pVarNum )
	...
	if ( restype < 0 )
	{
		//	some special script objects that definitely are not indexed resources
		if ( !strnicmp(pszSection, "QUEST", 5) )
		{
			//	ignore it on resync, since we do not have possibility (yet) to change quest
			if ( g_Serv.m_iModeCode == SERVMODE_Run )
				return true;

			//	load quest
			QuestDef *quest = new QuestDef();
			quest->r_Load(*pScript);
			g_Quests.push_back(quest);
			return true;
		}
	...
	if ( !rid.IsValidUID() )
	{
		g_Log.Error("Invalid %s block index '%s'\n", pszSection, pScript->GetArgStr());
		return false;
	}
	...
	case RES_ACCOUNT:	// NOTE: ArgStr is not the DEFNAME
		...
		return g_Accounts.Account_Load(pScript->GetArgStr(), *pScript);
	...
	case RES_SKILLCLASS:
		{
			...
			if ( pPrvDef )
			{
				pNewLink = dynamic_cast <SkillClass*>(pPrvDef);
			}
			else
			{
				pNewLink = new SkillClass(rid);
				m_ResHash.AddSortKey(rid, pNewLink);
			}
			CScriptLineContext LineContext = pScript->GetContext();
			pNewLink->r_Load(*pScript);
	...
	case RES_PROFESSION:
		{
			...
			if ( pPrvDef )
			{
				pNewLink = dynamic_cast <Profession*>(pPrvDef);
			}
			else
			{
				pNewLink = new Profession(rid);
				m_ResHash.AddSortKey(rid, pNewLink);
			}
			CScriptLineContext LineContext = pScript->GetContext();
			pNewLink->r_Load(*pScript);
	...
	if ( pNewLink )
	{
		...
		{
			g_Log.Error("Can't link resources in the world save file\n");
			return false;
		}
	...
	EXC_CATCH;
	EXC_DEBUG_START;
	g_Log.Debug("section '%s' args '%s'", pszSection, pScript->GetArgStr());
	EXC_DEBUG_END;
	return false;
}
...
RESOURCE_ID CResource::ResourceGetNewID( RES_TYPE restype, LPCTSTR pszName, VariableList::Variable ** ppVarNum, bool fNewStyleDef )
{
	...
	case RES_REGIONTYPE:
		{
			...
			if ( iPage > 255 )
				g_Log.Warn("Bad resource index page %d\n", iPage);
		}
		break;
	...
	if ( pszName )
	{
		...
		VariableList::Variable *pVarBase = g_Exp.m_VarDefs.GetKey( pszName );
		if ( pVarBase )
		{
			...
			rid.SetPrivateUID( pVarBase->GetValNum());
			if ( restype != rid.GetResType())
			{
				...
				case RES_PLEVEL:
					...
					break;
				default:
					g_Log.Error("Redefined name '%s' from %s to %s\n", pszName, GetResourceBlockName(rid.GetResType()), GetResourceBlockName(restype));
					return( ridinvalid );
				}
			}
			else if ( fNewStyleDef && pVarBase->GetValNum() != rid.GetPrivateUID() )
			{
				g_Log.Warn("Region redefines DEFNAME '%s' for another region!\n", pszName);
			}
	...
	if ( pszName )
	{
		...
		if ( iVarNum >= 0 )
			*ppVarNum = g_Exp.m_VarDefs.GetAt(iVarNum);
	}
...
void CResource::OnTick( bool fNow )
{
	...
	if ( m_fUseHTTP )
	{
		for ( int i = 0; i < m_WebPages.GetCount(); i++ )
		{
			...
			EXC_DEBUG_START;
			CWebPageDef * pWeb = static_cast <CWebPageDef *>(m_WebPages[i]);
			g_Log.Debug("web '%s' dest '%s' now '%d' index '%d'/'%d'\n",
...
void CResource::PrintEFOFFlags(bool bEF, bool bOF, CTextConsole *pSrc)
{
	...
	if ( bOF )
	{
		...
		if ( IsSetOF(OF_HouseMuteSpeech) ) catresname(zOptionFlags, "HouseMuteSpeech");

		if ( zOptionFlags[0] )
		{
			if ( pSrc ) pSrc->SysMessagef("Option flags: %s\n", zOptionFlags);
			else g_Log.Init("Option flags: %s\n", zOptionFlags);
		}

		if ( g_Cfg.m_iOptionFlags & ~VALID_OF )
			g_Log.Init("Mask 0%x of 0%x invalid, optional flags probably are messed up.\n",
				g_Cfg.m_iOptionFlags & ~VALID_OF, g_Cfg.m_iOptionFlags);
	}
	if ( bEF )
	{
		...
		if ( zExperimentalFlags[0] )
		{
			...
			else g_Log.Init("Experimental flags: %s\n", zExperimentalFlags);
		}

		if ( g_Cfg.m_iExperimental & ~VALID_EF )
			g_Log.Init("Mask 0%x of 0%x invalid, experimental flags probably are messed up.\n",
				g_Cfg.m_iExperimental & ~VALID_EF, g_Cfg.m_iExperimental);
	}
}
...
bool CResource::LoadIni( bool fTest )
{
	...
	if ( ! OpenResourceFind( m_scpIni, SPHERE_FILE ".ini", !fTest )) // Open script file
	{
		if ( ! fTest )
		{
			g_Log.Event( LOGL_FATAL|LOGM_INIT, "Can't open " SPHERE_FILE ".ini\n" );
		}
		return false;
	}
...
bool CResource::Load( bool fResync )
{
	...
	VERFILE_TYPE openMask = g_Install.OpenFiles(
		(1<<VERFILE_MAP)|(1<<VERFILE_STAIDX)|(1<<VERFILE_STATICS)|(1<<VERFILE_TILEDATA)|(1<<VERFILE_MULTIIDX)|(1<<VERFILE_MULTI)
		);
	if ( openMask != VERFILE_QTY )
	{
		g_Log.Event(LOGL_FATAL|LOGM_INIT, "The " SPHERE_FILE ".INI file is corrupt or missing\n");
		g_Log.Event(LOGL_FATAL|LOGM_INIT, "MUL File '%s' not found...\n", g_Install.GetBaseFileName(openMask));
		return false;
	}

	extern Tiledata *g_Tiledata;
	g_Tiledata = new Tiledata();
	if ( !g_Tiledata->open() )
	{
		g_Log.Event(LOGL_FATAL|LOGM_INIT, "MUL file '%s' not found...\n",
			g_Tiledata->name());
	...
	if ( !fResync )
	{
		...
		{
         	g_Log.Event(LOGL_FATAL|LOGM_INIT, "The " SPHERE_FILE ".INI file is corrupt or missing\n");
			g_Log.Event(LOGL_FATAL|LOGM_INIT, "Error opening table " SPHERE_FILE "tables...\n");
			return false;
		}
	...
	if ( !g_Cfg.ResourceGetDef(RESOURCE_ID(RES_SKILLCLASS, 0)) )
	{
		...
		SkillClass *pSkillClass = new SkillClass(RESOURCE_ID(RES_SKILLCLASS));
		m_ResHash.AddSortKey(RESOURCE_ID(RES_SKILLCLASS, 0), pSkillClass);
	}
	if ( !g_Cfg.ResourceGetDef(RESOURCE_ID(RES_PROFESSION, 0)) )
	{
		// must have at least 1 skill class.
		Profession *pProfession = new Profession(RESOURCE_ID(RES_PROFESSION));
		m_ResHash.AddSortKey(RESOURCE_ID(RES_PROFESSION, 0), pProfession);
	}
...
LPCTSTR CResource::GetDefaultMsg(long lKeyNum)
{
	...
	{
		g_Log.Error("Defmessage %d out of range [0..%d]\n", lKeyNum, DEFMSG_QTY-1);
		return "";
	}
