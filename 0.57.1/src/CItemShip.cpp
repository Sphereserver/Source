...
bool CItemShip::Ship_MoveDelta( CPointBase pdelta )
{
	...
	for ( int i=0; i <iCount; i++ )
	{
		...
		{
			g_Log.Error("Ship uid=0%x out of bounds\n", uid());
			continue;
		}
		pObj->MoveTo( pt );
...
bool CItemShip::r_Verb( CScript & s, CTextConsole * pSrc ) // Execute command from script
{
	...
	switch ( iCmd )
	{
		...
		case SHV_SHIPFACE:
		{
			...
			if ( ! s.HasArgs())
				return( false );
			return Ship_Face( GetDirStr( s.GetArgStr(), NULL ));
		}
		...
		case SHV_SHIPMOVE:
		{
			...
			if ( ! s.HasArgs())
				return( false );
			m_itShip.m_DirMove = GetDirStr( s.GetArgStr(), NULL );
...
CItem * CItemShip::GetShipPlank(int index)
{
	...
	for ( vector<UID>::iterator i = m_uidPlanks.begin(); i != m_uidPlanks.end(); ++i )
	{
