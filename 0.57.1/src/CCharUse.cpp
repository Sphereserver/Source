...
void CChar::Use_MoonGate( CItem * pItem )
{
	...
	//	moongate selects random target location
	if ( pItem->IsType(IT_MOONGATE))
		ptTeleport = g_Cfg.m_MoonGates[ Calc_GetRandVal(g_Cfg.m_MoonGates.GetCount()) ];
...
bool CChar::Use_Train_ArcheryButte( CItem * pButte, bool fSetup )
{
	...
	if ( GetDist( pButte ) < 2 )
	{
		...
		if ( pAmmoDef != NULL )
		{
			Emotef(g_Cfg.GetDefaultMsg( DEFMSG_ITEMUSE_ARCHB_REM ),
				pAmmoDef->GetName(),
...
bool CChar::Use_Repair( CItem * pItemArmor )
{
	...
	Emotef(g_Cfg.GetDefaultMsg(DEFMSG_REPAIR_MSG), pszText, pItemArmor->GetName());
...
CChar * CChar::Use_Figurine(CItem *pItem)
{
	...
	else
	{
		...
		if ( ! id )
		{
			...
			if ( !id )
			{
				g_Log.Error("FIGURINE id 0%x, no creature\n", pItem->GetDispID());
				return NULL;
			}
		}
...
bool CChar::Use_Seed( CItem * pSeed, CPointMap * pPoint )
{
	...
	{
		if ( pItem->IsType(IT_TREE) || pItem->IsType(IT_FOLIAGE) )
		{
			// already a tree here .
...
bool CChar::Use_Item( CItem * pItem, bool fLink )
{
	...
	//	Trigger same actions from all linked items
	if ( !fLink )
	{
		CItem *currentItem = pItem;
		long maxDepth = 64;
		while ( CItem *linkedItem = currentItem->m_uidLink.ItemFind() )
		{
			//	watch for possible deadloops
			if (( linkedItem == currentItem ) || ( linkedItem == pItem )) break;
			if ( --maxDepth == 0 ) break;

			currentItem = linkedItem;
			fAction |= Use_Item(currentItem, true);
		}
	}
