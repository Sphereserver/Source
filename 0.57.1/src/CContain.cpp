...
int CContainer::ContentConsume( RESOURCE_ID_BASE rid, int amount, bool fTest, DWORD dwArg , bool bTrade)
{
	// ARGS:
	...
	for ( CItem* pItem=GetContentHead(); pItem!=NULL; pItem=pItemNext)
	{
		...
		// Not consume newbie items, if flag bTrade is true (TRADECONSUME)
		if (( pItem->IsAttr(ATTR_NEWBIE) ) && ( bTrade == true ))
			continue;
		...
		if ( pCont != NULL )	// this is a sub-container.
		{
			...
			amount = pCont->ContentConsume( rid, amount, fTest, dwArg , bTrade);
			if ( amount <= 0 )
				break;
		}
	}
...
int CContainer::ContentCount( RESOURCE_ID_BASE rid, DWORD dwArg , bool bTrade)
{
	// Calculate total (gold or other items) in this recursed container
	return( INT_MAX - ContentConsume( rid, INT_MAX, true, dwArg , bTrade ));
}
...
void CContainer::ContentsDump( const CPointMap & pt, WORD wAttrLeave )
{
	...
	wAttrLeave |= ATTR_NEWBIE|ATTR_MOVE_NEVER;
	CItem * pItemNext;
	for ( CItem* pItem=GetContentHead(); pItem!=NULL; pItem=pItemNext)
	{
		pItemNext = pItem->GetNext();
...
int CContainer::ResourceConsume( const CResourceQtyArray * pResources, int iReplicationQty, bool fTest, DWORD dwArg , bool bTrade)
{
	...
	for ( int i=0; i<pResources->GetCount(); i++ )
	{
		...
		int iQtyCur = iQtyTotal - ContentConsume( rid, iQtyTotal, fTest, dwArg , bTrade);
		iQtyCur /= iResQty;
		if ( iQtyCur < iQtyMin )
			iQtyMin = iQtyCur;
	}
...
bool CContainer::r_WriteValContainer(LPCTSTR pszKey, CGString & sVal, CTextConsole *pSrc)
{
	...
	switch ( i )
	{
		...
		case 4:			//	traderescount
			sVal.FormatVal(*pKey ? ContentCount(g_Cfg.ResourceGetID(RES_ITEMDEF, pKey) , 0, true) : GetCount());
			break;

		case 5:			//	traderestest
		{
			CResourceQtyArray Resources;
			sVal.FormatVal(Resources.Load(pKey) ? ResourceConsume(&Resources, 1, true, 0, true) : 0);
		} break;
...
void CItemContainer::Trade_Status( bool fCheck )
{
	...
	if ( pChar1->IsClient())
	{
		...
		pChar1->GetClient()->xSend(&cmd, 0x11);
	}
	if ( pChar2->IsClient())
	{
		...
		pChar2->GetClient()->xSend(&cmd, 0x11);
	}
...
void CItemContainer::Trade_Delete()
{
	...
	if ( pChar->IsClient())
	{
		...
		cmd.SecureTrade.m_fname = 0;
		pChar->GetClient()->xSend(&cmd, 0x11);
	}
...
CPointMap CItemContainer::GetRandContainerLoc() const
{
	...
	for ( ; true; i++ )
	{
		...
		{
			i=0;
			g_Log.Error("unknown container gump id %d for 0%x\n", gump, GetDispID());
			break;
		}
...
insertit:
	...
	case IT_EQ_VENDOR_BOX:
		...
		{
			g_Log.Error("Un-equipped vendor box uid=0%x is bad\n", uid());
			break;
		}
		{
			...
			if ( pItemVend == NULL )
			{
				g_Log.Event( LOGL_WARN, "Vendor non-vendable item: %s uid=0%x, vendor: %s uid=0%x\n",
					pItem->GetResourceName(), pItem->uid(), GetContainer()->GetName(), GetContainer()->uid());
				pItem->Delete();
				break;
			}
