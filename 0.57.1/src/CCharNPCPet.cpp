...
void CChar::NPC_PetClearOwners()
{
	...
	//	going wild means changing name to the wild (not being renamed by person)
	if ( m_pNPC->m_renamed )
	{
		m_pNPC->m_renamed = false;
		SetName(Char_GetDef()->GetTypeName());
	}
...
bool CChar::NPC_CheckHirelingStatus()
{
	...
	int iPeriodWage = IMULDIV( iWage, iFoodConsumeRate, 24 * 60 * 6 * TICK_PER_SEC );
	if ( iPeriodWage <= 0 )
		iPeriodWage = 1;
