...
int CChar::NPC_GetTrainMax( const CChar * pStudent, SKILL_TYPE Skill ) const
{
	...
	return min(iMax, pStudent->Skill_GetMax(Skill));
}
...
bool CChar::NPC_CheckWalkHere( const CPointBase & pt, const CRegionBase * pArea, WORD wBlockFlags ) const
{
	...
	while ( CItem *pItem = AreaItems.GetItem() )
	{
		...
		int randInt = Calc_GetRandVal( Stat_GetAdjusted(STAT_INT));
		switch ( pItem->GetType() )
		{
		case IT_SHRINE:			// always avoid.
			return false;

		case IT_WEB:
			if ( GetDispID() == CREID_GIANT_SPIDER )
				continue;
			if ( Calc_GetRandVal(80) < randInt )
				return false;
			break;
		case IT_FIRE: // fire object hurts us ?
			if ( pCharDef->Can(CAN_C_FIRE_IMMUNE) )
				continue;
			if ( Calc_GetRandVal(20) < randInt )
				return false;
			break;
		case IT_SPELL:
			{
				...
				if ( Calc_GetRandVal(i) < randInt )
					return false;
				break;
			}
		case IT_MOONGATE:
		case IT_TELEPAD:
			if ( Calc_GetRandVal(10) < randInt )
				return false;
			break;
		}
	}
