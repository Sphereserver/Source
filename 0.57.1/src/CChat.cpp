...
not_in_a_channel:
		...
		g_Log.Event(LOGM_PLAYER_SPEAK, "%x:'%s' Says '%s' in chat to channel '%s'\n",
			pClient->socketId(), pClient->GetName(), szMsg, pChannel->GetName());
	...
	case 'e':	// e = Send a private message to ....
	{
		...
		g_Log.Event(LOGM_PLAYER_SPEAK, "%x:'%s' Says '%s' in chat privately to channel '%s'\n",
			pClient->socketId(), pClient->GetName(), szMsg, pChannel->GetName());
	...
	case 'z':	// z = emote
		if (!pChannel)
			goto not_in_a_channel;
		g_Log.Event(LOGM_PLAYER_SPEAK, "%x:'%s' Says '%s' in chat emoting to channel '%s'\n",
			pClient->socketId(), pClient->GetName(), szMsg, pChannel->GetName());
...
void CChat::WhereIs(CChatChanMember * pBy, LPCTSTR pszName ) const
{
	...
	{
		if ( !strcmp(client->GetChatName(), pszName) )
			continue;

		TEMPSTRING(pszMsg);
		if ( !client->IsChatActive() || !client->GetChannel() )
			sprintf(pszMsg, "%s is not currently in a conference.", pszName);
		else
			sprintf(pszMsg, "%s is in conference '%s'.", pszName, client->GetChannel()->GetName());
		CGString sFrom;
		DecorateName(sFrom, NULL, true);
		pBy->SendChatMsg(CHATMSG_PlayerTalk, sFrom, pszMsg);
		return;
...
void CChat::SendDeleteChannel(CChatChannel * pChannel)
{
	...
	while ( client = it.next() )
	{
		if ( client->IsChatActive() )
			client->addChatSystemMessage(CHATMSG_RemoveChannelName, pChannel->GetName());
	}
}
...
void CChat::SendNewChannel(CChatChannel * pNewChannel)
{
	...
	{
		if ( !client->IsChatActive() )
			client->addChatSystemMessage(CHATMSG_SendChannelName, pNewChannel->GetName(), pNewChannel->GetModeString());
	}
}
...
void CChat::DecorateName(CGString &sName, const CChatChanMember * pMember, bool fSystem) // static
{
	...
	if (!pMember || !pChannel)
		sName.Format("%d%s", iResult, "SYSTEM");
	else
		sName.Format("%d%s", iResult, (LPCTSTR) pMember->GetChatName());
}
...
void CChat::Broadcast(CChatChanMember * pFrom, LPCTSTR pszText, CLanguageID lang, bool fOverride)
{
	...
	while ( client = it.next() )
	{
		if ( client->IsChatActive() && ( fOverride || ( !fOverride && client->IsReceivingAllowed())) )
		{
			CGString sName;
			DecorateName(sName, pFrom, fOverride);
			client->SendChatMsg(CHATMSG_PlayerTalk, sName, pszText, lang );
		}
	}
}
