...
{
	// What is this char to the viewer ?
	VariableList::Variable *sVal = GetKey("NAME.HUE", true);
	if ( sVal )
	...
	switch ( Noto_GetFlag(pCharViewer, fIncog) )
	{
		...
		case NOTO_NEUTRAL:	return g_Cfg.m_iColorNotoNeutral;	// Gray 1 (someone that can be attacked)
		case NOTO_CRIMINAL:	return g_Cfg.m_iColorNotoCriminal;	// Gray 2 (criminal)
		case NOTO_GUILD_WAR:return g_Cfg.m_iColorNotoGuildWar;	// Orange (enemy guild)
		case NOTO_EVIL:		return g_Cfg.m_iColorNotoEvil;		// Red
	}

	return g_Cfg.m_iColorNotoDefault;	// Gray
}
...
LPCTSTR CChar::Noto_GetTitle() const
{
	...
	LPCTSTR pTitle = Noto_IsMurderer() ? "Murderer" : ( IsStatFlag(STATF_Criminal) ? "Criminal" :  g_Cfg.GetNotoTitle(Noto_GetLevel()) );
...
#define	NOTO_FACTOR		(300/NOTO_DEGREES)
	...
	int iDegree = min(abs(iDelta) / NOTO_FACTOR, 7);
...
void CChar::Noto_Fame( int iFameChange )
{
	...
	int iFame = max(Stat_GetAdjusted(STAT_FAME), 0);
...
void CChar::Noto_Kill(CChar * pKill, bool fPetKill, int iOtherKillers)
{
	...
	if ( NotoThem != NOTO_GUILD_SAME )			// no fame/karma gain for killing guildmates
	{
		...
		if ( !fPetKill )							
		{
			...
			if ( g_Cfg.m_iExperienceMode && ( g_Cfg.m_iExperienceMode&EXP_MODE_RAISE_COMBAT ))
			{
...
CItemMemory * CChar::Memory_CreateObj( UID uid, WORD MemTypes )
{
...
CItemMemory * CChar::Memory_FindObj( UID uid ) const
{
...
CItemMemory * CChar::Memory_AddObjTypes( UID uid, WORD MemTypes )
{
...
bool CChar::CheckCrimeSeen( SKILL_TYPE SkillToSee, CChar * pCharMark, const CObjBase * pItem, LPCTSTR pAction )
{
	...
	// Who notices ?
	CWorldSearch AreaChars(GetTopPoint(), UO_MAP_VIEW_SIGHT, this);
	while (CChar * pChar = AreaChars.GetChar())
	{
		if ( !pChar->CanSeeLOS(this) ) continue;
		...
		if ( pAction != NULL )
		{
			...
			else
			{
				sprintf(z, g_Cfg.GetDefaultMsg(DEFMSG_YOUNOTICE_2),
					GetName(), pAction, fYour ? g_Cfg.GetDefaultMsg( DEFMSG_YOUNOTICE_YOUR ) : pCharMark->GetName(),
...
cantsteal:
	...
	if ( pContainer )
	{
		if ((LAYER_TYPE)pItem->GetContainer()->GetContainedLayer()==LAYER_VENDOR_STOCK)
		{
			SysMessageDefault( DEFMSG_STEALING_VENDOR );
			return -SKTRIG_ABORT;
		}
...
void CChar::CallGuards( CChar * pCriminal )
{
	...
	{
		bSearchedGuard = true;
		CWorldSearch AreaCrime(GetTopPoint(), UO_MAP_VIEW_SIGHT, this);
		while ( CChar *pChar = AreaCrime.GetChar() )
		{
	...
	VariableList::Variable * pVar = pCriminal->m_pArea->m_TagDefs.GetKey("OVERRIDE.GUARDS");
...
#define USEARMORMAX(_x_)			ArmorRegionMax[ARMOR_##_x_] = max(ArmorRegionMax[ARMOR_##_x_], iDefense)
#define USEARMORMAX2(_x_,_y_)		ArmorRegionMax[ARMOR_##_x_] = max(ArmorRegionMax[ARMOR_##_x_], iDefense); \
		ArmorRegionMax[ARMOR_##_y_] = max(ArmorRegionMax[ARMOR_##_y_], iDefense)
#define USEARMORMAX4(_a_,_b_,_c_,_d_)	ArmorRegionMax[ARMOR_##_a_] = max(ArmorRegionMax[ARMOR_##_a_], iDefense); \
		ArmorRegionMax[ARMOR_##_b_] = max(ArmorRegionMax[ARMOR_##_b_], iDefense); \
		ArmorRegionMax[ARMOR_##_c_] = max(ArmorRegionMax[ARMOR_##_c_], iDefense); \
		ArmorRegionMax[ARMOR_##_d_] = max(ArmorRegionMax[ARMOR_##_d_], iDefense)
...
int CChar::CalcArmorDefense() const
{
	...
	for ( CItem* pItem=GetContentHead(); pItem!=NULL; pItem=pItem->GetNext())
	{
		...
		switch ( pItem->GetEquipLayer() )
		{
			...
			case LAYER_CAPE:		// 20 = cape
				USEARMORMAX2(BACK, ARMS);
				break;
			case LAYER_ROBE:		// 22 = robe over all.
				USEARMORMAX4(CHEST, BACK, ARMS, LEGS);
				break;
			case LAYER_LEGS:
				USEARMORMAX2(LEGS, FEET);
				break;
			case LAYER_HAND2:
	...
	return max(( iDefenseTotal / 100 ) + m_ModAr, 0);
}
#undef USEARMORMAX4
#undef USEARMORMAX2
#undef USEARMORMAX
...
int CChar::OnTakeDamage( int iDmg, CChar * pSrc, DAMAGE_TYPE uType )
{
	...
	//	players have some magic resistance attributes
	if ( m_pPlayer )
	{
		int percentRemoved = 0;
		if ( uType & DAMAGE_COLD )
			percentRemoved += m_pPlayer->m_resCold;
		if ( uType & DAMAGE_ENERGY )
			percentRemoved += m_pPlayer->m_resEnergy;
		if ( uType & DAMAGE_FIRE )
			percentRemoved += m_pPlayer->m_resFire;
		if ( uType & DAMAGE_POISON )
			percentRemoved += m_pPlayer->m_resPoison;

		percentRemoved = min(100, max(percentRemoved, 0));
		iDmg -= ( iDmg * percentRemoved ) / 100;
	}
...
effect_bounce:
	...
	int la;
	for ( la = m_lastAttackers.size()-1; la >= 0; la-- )
	{
		LastAttackers *l = &m_lastAttackers.at(la);
		if ( l->charUID == pSrc->uid() )
		{
			l->elapsed = 0;
			l->amountDone += iDmg;
			break;
	...
	{
		PacketCombatDamage *cmd = new PacketCombatDamage(iDmg, this, pSrc);
		delete cmd;
	}
...
int CChar::OnTakeDamageHitPoint( int iDmg, CChar * pSrc, DAMAGE_TYPE uType )
{
		...
		! (uType & (DAMAGE_GENERAL|DAMAGE_GOD)))
	{
		...
		if ( pSrc != this )
		{
			...
			{
				SysMessagef( ppMsg[0], ( pSrc == NULL ) ? "It" : pSrc->GetName());
			}
	...
	//	do not apply armor if ignorearmor is set
	if ( IsSetMagicFlags(MAGICF_IGNOREAR) && ( uType & DAMAGE_MAGIC ) )
	{
		return iDmg;
	}

	return ( iDmg - Calc_GetRandVal(iMaxCoverage) );
}
...
void CChar::Memory_Fight_Retreat( CChar * pTarg, CItemMemory * pFight )
{
	...
	SysMessagef( fCowardice ?
		...
		g_Cfg.GetDefaultMsg( DEFMSG_COWARD_2 ), pTarg->GetName());
...
void CChar::Memory_Fight_Start( const CChar * pTarg )
{
	...
	if ( IsClient() )
	{
		PacketSwing *packet = new PacketSwing(GetClient(), pTarg->GetUID().CharFind());
	}
...
WAR_SWING_TYPE CChar::Fight_Hit( CChar * pCharTarg )
{
	...
	if ( CanSee(pCharTarg) )
	{
		if ( (pCharTarg->m_pNPC && pCharTarg->IsStatFlag(STATF_Ridden) ) || !CanSeeLOS(pCharTarg) )
	...
	if ( skill == SKILL_ARCHERY )
	{
		...
		if ( dist < iMinDist )
		{
			...
			UpdateAnimate(9, false, iTime/TICK_PER_SEC );
			return( WAR_SWING_EQUIPPING );
		}
	...
	int iWeaponWeight = min((( pWeapon ) ? ( pWeapon->GetWeight()/WEIGHT_UNITS + 1 ) : 1 ), 10);
	...
	if ( m_Act_Difficulty < 0 )
	{
		if ( IsClient() )
		{
			int idx = GetClient()->m_TagDefs.GetKeyNum("SpecialCombatMove", true);
			if ( idx )
				CombatAbility::abils[idx]->OnMiss(this, pCharTarg);
		}

		if ( IsTrigUsed(TRIGGER_HITMISS) )
		{
			CScriptTriggerArgs	Args(pWeapon);
	...
	DAMAGE_TYPE uType = ( DAMAGE_HIT_BLUNT | DAMAGE_HIT_PIERCE | DAMAGE_HIT_SLASH );

	if ( IsClient() )
	{
		int idx = GetClient()->m_TagDefs.GetKeyNum("SpecialCombatMove", true);
		if ( idx )
			CombatAbility::abils[idx]->OnHit(this, pCharTarg, iDmg, uType);
	}

	CScriptTriggerArgs	Args( iDmg, uType, pWeapon );
	...
	iDmg = pCharTarg->OnTakeDamage( iDmg, this, uType);
	if ( iDmg > 0 )
	{
