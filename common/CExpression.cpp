...
{
	ADDTOCALLSTACK("CVarDefArray::FindValStr");
	int iQty = GetCount();
...
{
	ADDTOCALLSTACK("CVarDefArray::FindValNum");
...
{
	ADDTOCALLSTACK("CVarDefArray::Copy");
	if ( this == pArray )
		return;

	//	we need empty() here for "call" to work
	Empty();
...
{
	ADDTOCALLSTACK("CVarDefArray::SetNumNew");
...
{
	ADDTOCALLSTACK("CVarDefArray::SetNum");
	ASSERT(pszName);
	if ( pszName[0] == '\0' )
		return( -1 );
...
{
	ADDTOCALLSTACK("CVarDefArray::SetStr");
...
{
	ADDTOCALLSTACK("CVarDefArray::GetKey");
	if ( pszKey )
	{
...
int CVarDefArray::GetKeyNum( LPCTSTR pszKey, bool fZero  ) const
{
	ADDTOCALLSTACK("CVarDefArray::GetKeyNum");
...
{
	ADDTOCALLSTACK("CVarDefArray::GetKeyStr");
	CVarDefBase * pVar = GetKey(pszKey);
	if ( pVar == NULL )
		return (fZero ? "0" : "");
...
{
	ADDTOCALLSTACK("CVarDefArray::GetParseKey");
...
{
	ADDTOCALLSTACK("CVarDefArray::GetParseVal");
...
void CVarDefArray::r_WritePrefix( CScript & s, LPCTSTR pszPrefix )
{
	ADDTOCALLSTACK("CVarDefArray::r_WritePrefix");
	LPCTSTR		pszVal;
...
{
	ADDTOCALLSTACK("CVarDefArray::DumpKeys");
...
{
	ADDTOCALLSTACK("CVarDefArray::ClearKeys");
	// Clear the tag array
	for ( int i=GetCount(); --i >= 0;)
	{
		if ( mask && *mask )
