...
BYTE CChar::GetHeightMount( bool fEyeSubstract ) const
{
	...
	//DEBUG_ERR(("Height %d\n",Height));
	return (( IsStatFlag(STATF_OnHorse) ) ? ( Height + 4 ) : (  ((fEyeSubstract ) ? ( --Height ) : ( Height )) )); //if mounted +4, if not -1 (let's say it's eyes' height)
}
...
void CChar::InitPlayer( const CEvent * pBin, CClient * pClient )
{
	...
	bool mSex = false; // Male
	bool mElf = false; // Human
	...
	if ( pClient->GetAccount() && pClient->GetAccount()->GetResDisp() >= RDS_ML )
	{
		...
		{
			mElf = true;
			mSex = ((pBin->Create.m_sex - 2) == 1 );
		}
		else
		{
			mSex = ( pBin->Create.m_sex == 1 );
		}
	}
	else
		mSex = ( pBin->Create.m_sex == 1 );
	...
	if ( mElf )
		SetID( (CREID_TYPE) g_Cfg.ResourceGetIndexType( RES_CHARDEF, ( mSex ) ? "c_elf_female" : "c_elf_male" ));
	else
		SetID( (CREID_TYPE) g_Cfg.ResourceGetIndexType( RES_CHARDEF, ( mSex ) ? "c_woman" : "c_man" ));
	...
	if ( bNameIsAccepted )
		...
		SetNamePool( ( mSex ) ? "#NAMES_HUMANFEMALE" : "#NAMES_HUMANMALE" );
	...
	int _str = minimum((pBin->Create.m_str + 1), 80);
	int _dex = minimum((pBin->Create.m_dex + 1), 80);
	int _int = minimum((pBin->Create.m_int + 1), 80);
	...
	int skill1 = minimum(pBin->Create.m_val1, 50);
	int skill2 = minimum(pBin->Create.m_val2, 50);
	int skill3 = minimum(pBin->Create.m_val3, 50);
	...
	for ( i=0; i<4; i++ )
	{
		...
		{
			iSkill = ( mSex ) ? RES_NEWBIE_FEMALE_DEFAULT : RES_NEWBIE_MALE_DEFAULT;
		}
			...
			if ( *pszKey == '.' )
			{
				...
				if( strnicmp(pszKey, "MAX", 3) ) {
					...
					for ( int iAttacker = 0; iAttacker < m_lastAttackers.size(); iAttacker++ ) {
						LastAttackers *attacker = &m_lastAttackers.at(iAttacker);
						if(( maxDamage == NULL ) && ( attacker->amountDone > 0 )) {
							maxDamage = attacker;
							attackerIndex = iAttacker;
						}
						if(( maxDamage != NULL ) && ( maxDamage->amountDone < attacker->amountDone )) {
							maxDamage = attacker;
							attackerIndex = iAttacker;
						}
					}
